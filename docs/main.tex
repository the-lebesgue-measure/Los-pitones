...
Demostración identidad:
\[
\log\left( \sum e^{x_i} \right)=\log\left( \sum e^{x_i-\max(x)+\max(x)} \right)=\log\left( e^{\max(x)} \sum e^{x_i-\max(x)} \right)
\]
\[
=\max(x) + \log\left( \sum_i=1^n e^{x_i-\max(x)} \right)
\]
